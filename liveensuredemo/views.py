...
def index(request):
    if (request.method == 'POST'):
        ...
        print("hey->", request.session['email'])
...
def device(request):
    ...
    if ("api_key" not in request.session or "api_password" not in request.session or "agent_id" not in request.session):
        ...
        return render(request, "liveensure/device.html",
                      {"agentId": request.session['agent_id'], "email": request.session['email'],"host": _getHost(), "version": c['VERSION']})
...
def behaviour(request):
    if ("api_key" not in request.session or "api_password" not in request.session or "agent_id" not in request.session):
        ...
        return render(request, "liveensure/behaviour.html",
                      {"agentId": request.session['agent_id'], "host": _getHost(), "version": c['VERSION']})
...
def location(request):
    if ("api_key" not in request.session or "api_password" not in request.session or "agent_id" not in request.session):
        ...
        return render(request, "liveensure/location.html",
                      {"agentId": request.session['agent_id'], "host": _getHost(), "map_key": _getMapKey(request),
                       "version": c['VERSION']})
